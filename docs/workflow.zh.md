...
```
x = ceil(170000 ÷ (12000 × 0.8)) = ceil(170000 ÷ 9600) = 18 轮
```

`--cap` = 模型单轮输出上限（中文字符，默认 12000）；`--util` 默认 0.8。
x 写进 `rounds_planned`，此后每轮显示 `轮 n/x`。**跑满 x 轮 ≠ 完成**，验收只看 `gate`。
...
| `AUDIT` | 字数刚达标 | — | 跑全部闸门（`gate` 自动） |
| `REPAIR` | 有形式问题 | 2 | 按清单修，不写新内容 |
| `BLOCKED` | 重号/G10/交付物损坏 | 2 | 先处理，再重跑 |
| `DONE` | 全过 | 0 | 才可以停 |
...
给出五种口径：汉字 / 含全角标点 / 去空白 / 总字符 / 交付 docx。
`gate` 判定时取 **`min(源文件汉字, docx 汉字)`**，防止“源文件够但 docx 漏内容”被误判达标。
