...
### M0.1 无人值守（睡觉时也能跑完，v11.1）

**前提事实**：Chatbox 没有"自动进入下一回合"的开关。回合只在"发出不含工具调用的消息"时结束，
而**结束 = 停下来等你输入**。所以要让机器在你睡觉时继续跑，必须用下面两条一起上：

1. **回合内不停（M0）**：绝不发非工具消息 → 一个回合内尽可能多写几块，直到撞上上下文/输出上限。
2. **把"消息队列"当自动继续**：Chatbox 支持排队消息——**当前回复结束后，队列里的下一条会自动发出**
   （见 1.23 更新说明："it lines up behind the current reply… will send after the current response finishes"）。
   睡前在输入框连续放入 **N 条** `继续（按 v11 M0 执行，不许收尾）`。队列上限 **20** 条。
   这样每结束一个回合，客户端会自动发出下一条 → 相当于自动续跑约 20 个回合。
   现成模板：`templates/queue_resume.txt`（把它粘进输入框，每条回车一次即可入队）。

**计算能不能一夜跑完**：
`一夜产出 ≈ 队列条数 N × 每回合块数 k × 每块字数 c`。
例：N=20、k=6、c≈1,800 → 约 216,000 字，足够一本 20 万字的第五幕。

**绕不开的硬边界**（谁的开关都改不了）：单次响应输出上限、上下文窗口、总输出量 x。
所以"一夜跑完"= 队列 20 条 × 每回合多块；若还不够，醒来后再补一轮队列接着跑。

**真正的零人工路径**：如果你有可调用的 LLM API Key，用 `scripts/overnight_driver.py`
在聊天之外跑循环（读骨架 → 逐块生成 → 落盘 → `gate` → 不够继续），不受回合/上下文约束。
详见 `docs/unattended.zh.md` / `docs/unattended.en.md`。
...
## 13. 更新日志
### v11.1.0
- **新增 M0.1《无人值守》**：把 Chatbox 的"消息队列"当自动继续用——队列里的消息会在当前回合结束后
  自动发出，睡前排队 N 条（上限 20）即可自动续跑约 20 个回合。给出"一夜产出"估算公式与硬边界。
- 新增 `templates/queue_resume.txt`（20 条现成"继续"）、`docs/unattended.zh.md` / `.en.md`。
- 新增 `scripts/overnight_driver.py`：有 API Key 时的真正零人工驱动（聊天之外跑循环）。
