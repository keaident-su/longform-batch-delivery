...
## v10 相对 v9 新增了什么（"跑完一轮就停"的真修复）

先纠正一个被传错的说法：**"回合边界由客户端决定，技能改不了"——这是错的。**

回合只在你输出一条**不带工具调用**的消息时结束。证据：同一回合里能连续发生十几次工具调用。
所以"跑完一块就停"有四个原因，性质完全不同：

| 真原因 | 性质 | 能不能修 |
|---|---|---|
| 模型写完一块就想收尾（**v8/v9 的 M2 甚至要求它收尾**） | 行为习惯 | ✅ **v10 已修** |
| 客户端 `pauseOnToolCallLimit` | 配置开关 | ✅ 改配置即可 |
| 上下文窗口 / 单次响应上限 | 物理边界 | ❌ 只能管理（开 `autoCompaction`） |
| 总输出量 x | 内容量 | ❌ 不应改 |

**v10 修的两处：**

1. **把 M2 改对了。** 原文是"每条回复必须以 `✅ DONE` 或 `⏩ RESUME` 结尾"——这条规则**自己就在逼模型收尾**。
   现在改成：**只有回合真的要结束时才写结尾**，回合内不得输出任何汇总/表格/结语。
2. **密度优先。** 实测一段"看似 300 字"的对白，净增汉字只有 ~60。
   新增 `density` 命令：`汉字/段 ≥ 120`、`对白行占比 ≤ 50%`，不合格 exit 2。

```bash
python scripts/run_state.py density
```

`chunks_per_turn` 默认从 3 提到 **8**。
...
**一句话：在 `python scripts/run_state.py gate` 退出码变成 `0` 之前，绝不发出任何"不带工具调用"的消息。**

工作模式是"思考 → 调工具 → 读结果 → 再决定"的循环。**一条不含工具调用的消息 = 回合结束 = 把方向盘交回用户。**
所以"用了技能还是跑一轮就停"的真因，不是 25 步护栏（那是回合内的检查点，Full Access + 关掉"每 25 步暂停"后已不弹），
而是模型自己写了一篇"汇报"。

### 回合进行中只允许一件事

✅ 发起工具调用：`write_file` 写正文 / `edit_file` 改稿 / `run_command` 校验与账本。

### 明确禁区（出现任一即违反）

| 禁区 | 例子 |
|---|---|
| 汇报表 | "## 第 N 批已交付…目标/当前/还差" |
| 征询与提议 | "要不要我…""下一步请你选""如果你希望…" |
| 追问式结尾 | 任何以问号结尾、等待用户输入的句子 |
| 收尾语 | "本轮到此""先到这里""小结一下""以上" |
| 正文入聊天 | 正文一律 `write_file` 落盘 |
...
`gate` 非 0：**不汇报、不提问、不总结，直接发起下一次工具调用。**
只有撞到硬边界（上下文用尽 / 单次响应输出上限）才写一行 `⏩ RESUME` 后立即结束。

### 自查

```bash
python _selftest/loop_guard.py     # 退出码 0=就绪 / 2=未就绪
```

它检查：SKILL.md 是否含 M0、`gate exit 0` 是否为唯一收尾条件、description 双口径 ≤1024、
以及客户端 `pauseOnToolCallLimit` 是否为 `false`。
